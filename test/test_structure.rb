...
class TestStructure < Test::Unit::TestCase
  ...
  def test_instantiation_with_more_arguments_throws_error
    assert_raise ArgumentError do
      Customer.new('Joe Smith', '123 Maple, Anytown NC', 12_345, 'extra param')
    end
  end
  ...
  def test_inspect
    assert_equal(
      '#<Teststruct Customer name="Joe Smith", address="123 Maple, Anytown NC", zip=12345>',
      @joe.inspect
    )

    Structure.new('Customer', :name, :address, :zip)
    joe = Structure::Customer.new('Joe Smith', '123 Maple, Anytown NC', 12_345)
  ...
  def test_members
    assert_equal(%i[name address zip], @joe.members)
  end
  ...
  def test_select
    ...
    assert_equal([22, 44, 66], l.select(&:even?))
  end
  ...
  def test_values_at
    assert_equal(['Joe Smith', 12_345], @joe.values_at(0, 2))
  end
end
